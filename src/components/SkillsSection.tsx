...
const dev = (name: string, variant: 'original' | 'plain' | 'original-wordmark' | 'plain-wordmark' = 'original') =>
...
const categories: Category[] = [
  ...
  },
  {
    ...
    skills: [
      { name: 'Talend',   icon: 'https://upload.wikimedia.org/wikipedia/commons/9/97/Talend_logo.svg' },
  ...
  },
  {
    ...
    skills: [
      ...
      { name: 'K6',         icon: 'https://avatars.githubusercontent.com/u/54012786?s=200&v=4' },
  ...
  },
  {
    ...
    skills: [
      ...
      { name: 'PowerPoint', icon: 'microsoft-powerpoint-2013-logo-svgrepo-com.svg' },
  ...
  return (
    ...
    >
      <div className="rounded-2xl border overflow-hidden h-full dark:border-white/10 border-black/10 dark:bg-white/[0.03] bg-black/[0.03]">
        {/* Header */}
        <div className={`px-5 py-3 bg-linear-to-r ${category.headerGradient} border-b dark:border-white/10 border-black/10 flex items-center gap-2`}>
          <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: category.accent }} />
          <h3 className="text-sm font-semibold textTwo tracking-wide">{category.title}</h3>
        </div>
